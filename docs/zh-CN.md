...
由于 sessionless 连接建立后不会再有任何远端输出，wrapper 会自己确认并
报告连接状态。连接尝试进行期间，它以较短的间隔重复执行 `ssh -O check`，
第一次确认 ControlMaster 建立时，清空窗口并打印一行提示，说明交互操作应
使用哪个别名。这次清屏同时抹掉整个认证交互——包括堡垒机回显的验证码。
JumpOTP 自身从不打印验证码，但无法阻止服务端回显，而该窗口可能在无人值守
的情况下停留数天。清屏发生在「确认建立」而不是「提交验证码」时刻，因为
服务端回显是一次网络往返，可能在本地写入返回之后才到达。始终没有确认到
master 的尝试不会清屏，其失败信息会保留在屏幕上。
